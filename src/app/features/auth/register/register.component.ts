...
@Component({
  ...
  styles: [`
    ...
    .line-input.err { border-bottom-color: #dc2626; }
    .google-btn { transition: transform 0.15s, box-shadow 0.15s; }
    ...
    .strength-bar { height: 3px; flex: 1; border-radius: 2px; transition: background-color 0.4s; }
  `],
  template: `
    <div>
      <!-- Header -->
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" novalidate>
        ...
        <div class="mb-6">
          <label class="block text-xs tracking-widest uppercase mb-2"
                 style="color:#a89585; letter-spacing:0.15em; font-weight:500;" for="name">
            Full Name
          </label>
          <input id="name" type="text" formControlName="name"
            autocomplete="name" placeholder="e.g. Priya Rathod"
            class="line-input" [class.err]="isInvalid('name')"
            (focus)="focusedField='name'" (blur)="focusedField=''" />
          @if (isInvalid('name')) {
            <p class="text-xs mt-1" style="color:#dc2626; font-weight:300;">{{ getFieldError('name') }}</p>
          }
        </div>
        ...
        <div class="mb-6">
          <label class="block text-xs tracking-widest uppercase mb-2"
                 style="color:#a89585; letter-spacing:0.15em; font-weight:500;" for="reg-email">
            Email Address
          </label>
          <input id="reg-email" type="email" formControlName="email"
            autocomplete="email" placeholder="you@example.com"
            class="line-input" [class.err]="isInvalid('email')"
            (focus)="focusedField='email'" (blur)="focusedField=''" />
          @if (isInvalid('email')) {
            <p class="text-xs mt-1" style="color:#dc2626; font-weight:300;">{{ getFieldError('email') }}</p>
          }
        </div>
        ...
        <div class="mb-6">
          <label class="block text-xs tracking-widest uppercase mb-2"
                 style="color:#a89585; letter-spacing:0.15em; font-weight:500;" for="reg-password">
            Password
          </label>
          <div class="relative">
            <input id="reg-password" [type]="showPassword() ? 'text' : 'password'"
              ...
              class="line-input pr-6" [class.err]="isInvalid('password')"
              (focus)="focusedField='password'" (blur)="focusedField=''" />
            <button type="button" (click)="showPassword.set(!showPassword())"
              class="absolute right-0 top-1/2 -translate-y-1/2" style="color:#a89585;">
              @if (showPassword()) {
          ...
          @if (isInvalid('password')) {
            <p class="text-xs mt-1" style="color:#dc2626; font-weight:300;">{{ getFieldError('password') }}</p>
          }
          <!-- Strength bars -->
          @if (form.get('password')?.value) {
            <div class="mt-2.5 flex gap-1.5 items-center">
              @for (bar of strengthBars(); track $index) {
                <div class="strength-bar" [style.background-color]="bar"></div>
              }
              <span class="text-xs ml-1" style="color:#a89585; font-weight:300;">{{ strengthLabel() }}</span>
            </div>
          }
        </div>
        ...
        <div class="mb-8">
          <label class="block text-xs tracking-widest uppercase mb-2"
                 style="color:#a89585; letter-spacing:0.15em; font-weight:500;" for="confirmPassword">
            Confirm Password
          </label>
          <input id="confirmPassword" [type]="showPassword() ? 'text' : 'password'"
            formControlName="confirmPassword" autocomplete="new-password" placeholder="Repeat your password"
            class="line-input"
            [class.err]="form.hasError('passwordMismatch') && form.get('confirmPassword')?.touched"
            (focus)="focusedField='confirmPassword'" (blur)="focusedField=''" />
          @if (form.hasError('passwordMismatch') && form.get('confirmPassword')?.touched) {
            <p class="text-xs mt-1" style="color:#dc2626; font-weight:300;">Passwords do not match.</p>
          }
        </div>
        ...
        @if (fieldErrors().length > 0) {
          <div class="mb-5 px-4 py-3 text-sm"
            ...
            <ul class="list-disc list-inside space-y-1">
              @for (err of fieldErrors(); track err.field) {
                <li>{{ err.message }}</li>
              }
            </ul>
          </div>
        ...
        <p class="text-xs text-center mt-4" style="color:#b8a898; font-weight:300;">
          By creating an account you agree to our
          <a href="#" style="color:#7b1c1c; text-decoration:underline; text-underline-offset:2px;">Terms</a>
          &amp;
          <a href="#" style="color:#7b1c1c; text-decoration:underline; text-underline-offset:2px;">Privacy Policy</a>.
        </p>
      </form>

      <p class="text-center text-sm mt-5" style="color:#a89585; font-weight:300;">
...
export class RegisterComponent {
  ...
  fieldErrors  = signal<{ field: string; message: string }[]>([]);
  focusedField = '';
  ...
  strengthBars(): string[] {
    ...
    return Array.from({ length: 4 }, (_, i) => (i < score ? colors[score - 1] : '#e5e7eb'));
  }
  ...
  strengthLabel(): string {
    ...
    if (/[0-9]/.test(pass))        score++;
    if (/[^A-Za-z0-9]/.test(pass)) score++;
    return ['Weak', 'Fair', 'Good', 'Strong'][score - 1] || '';
  }
  ...
  getFieldError(field: string): string {
    ...
    if (ctrl.hasError('maxlength'))  return `Maximum ${ctrl.getError('maxlength').requiredLength} characters.`;
    if (ctrl.hasError('minlength'))  return `Minimum ${ctrl.getError('minlength').requiredLength} characters required.`;
